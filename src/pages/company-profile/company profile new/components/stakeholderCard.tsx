...
const StakeholderCards = (props: StakeholderCardsProps) => {
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-12">Top Performers</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
        {props.data.map((stakeholder, index) => (
          <div
            key={index}
            className="border border-black rounded-lg p-8 flex flex-col items-center justify-center bg-white"
          >
            {stakeholder.name ? (
              <>
                {/* Profile Section */}
                <div className="flex flex-col items-center gap-2 mb-8 w-full">
                  <img
                    src={getUserImageSrc(stakeholder.image)}
